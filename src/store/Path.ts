...
import { PathItem, LoaderInterdface, PathItemWithPoints } from 'types';
...
export interface PathInterface extends LoaderInterdface {
  createPath: (data: Omit<PathItemWithPoints, 'id'>) => void;
...
class Path implements PathInterface {
  ...
  async createPath(path: Omit<PathItemWithPoints, 'id'>) {
    this.startLoad();
    ...
    try {
      ...
      dialogStore.close();
      pathsStore.addPath = { id: result.data, ...data };
    } catch (e) {
